...
class Orchestrator(object):
    ...
    def initialize_nfvi(self, nfvo_host, nfvo_port, vnfm_port):
        ...
        #ts = time.time()
        #deploy_log = open("deploy_log", "a")
        #deploy_log.write(str(ts)+":admin_net\n")   
	...
	vdu_instance = self.create_vdu_instance(graph, vdu_name)
        ...
        ts = time.time()
        deploy_log = open("deploy_log", "a")
        deploy_log.write(str(ts)+":"+str(vdu_name)+"_"+str(vdu_instance._infra_id[0:12])+"\n")   #+str(vdu_name)+"_"
        return vnf_instance
	...
	for policy in self.policies:
            if policy.type == 'tosca.policies.nfv.doctor.ndn.security.update_firewall':
                ...
                if action is not None:
                    ...
                    deploy_log = open("deploy_log", "a")         
                    ts = time.time()
                    deploy_log.write(str(ts)+":CONTENT_BLOCKED_"+str(to_install[0])+"_"+str(target_firewall)+"\n")
    ...
    def handle_cpa_alert(self, probe_id):
        ...
        deploy_log = open("deploy_log", "a")         
        ts = time.time()
        deploy_log.write(str(ts)+":CPA_DETECTED_"+str(router)+"\n")
            ...
            if policy.type == 'tosca.policies.nfv.doctor.ndn.scaling':
                for target in policy.targets:
                    if target == vnf:
                        ...
                        pit_log = open("pit_log", "a")         
                        ts = time.time()
                        pit_log.write(str(ts)+":"+str(vnf)+"_"+str(ip)+":"+str(count_metric)+"\n")
    ...
    def apply_scaling_policy(self, target, replicas):
        ...
        deploy_log = open("deploy_log", "a")         
        ts = time.time()
        deploy_log.write(str(ts)+":SCALE_"+str(target)+"\n")
        # TODO: target VDU, delete it from VNFM
